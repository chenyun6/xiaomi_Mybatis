...
<head>
	...
	<script type="text/javascript">
		function deleteOrder(orderId) {
			if(confirm("确定删除订单？")){
				location.href="${pageContext.request.contextPath}/orderservlet?method=deleteByid&oid="+orderId;
			}
		}
			...
			<table id="tb_list" class="table table-striped table-hover table-bordered table-condensed">
				...
				<c:forEach items="${orderList}" var="order" varStatus="i">
					<tr>
						...
						<th>
							<button type="button" class="btn btn-default btn-sm" onclick="deleteOrder('${order.id}')" >删除订单</button>
