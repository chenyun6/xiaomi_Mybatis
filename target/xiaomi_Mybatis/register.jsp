...
<script type="text/javascript">
	...
	$(function(){
		$("#valiImg").click(function () {
			$("#valiImg").attr("src","${pageContext.request.contextPath}/userservlet?method=getValiCode&n="+Math.random());
		});
		...
		$("#valicode").change(function(){
			$.post("userservlet?method=checkValiCode","code="+this.value,function (data) {
				if(data=="1"){
					$("#valicodeMsg").html("验证码输入错误").css("color","red");
					$("#registerBtn").attr("disabled",true);
				}else {
					$("#valicodeMsg").html("验证码正确").css("color","green");
					$("#registerBtn").removeAttr("disabled");
				}
			})
		});
			...
			<div class="center-block" style="margin-top: 80px;">
				<form id="form1" class="form-horizontal" action="userservlet?method=register" method="post">
					...
					<div class="form-group">
						...
						<div class="col-sm-8" style="width: 40%">
							<input type="text" style="width: 60%;" id="valicode" class="form-control col-sm-10"
								   placeholder="验证码" />
							<img id="valiImg" style="width: 40%;height: 34px" src="${pageContext.request.contextPath}/userservlet?method=getValiCode">
						</div>
						<div class="col-sm-2">
								<p class="text-danger"><span class="help-block " id="valicodeMsg"></span></p>
						</div>
					</div>
					<hr>
					<div class="form-group">
